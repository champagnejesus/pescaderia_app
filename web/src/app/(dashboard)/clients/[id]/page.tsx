  ...
  if (clientLoading) {
    return (
      <div className="min-h-screen bg-abyssal-bg flex flex-col">
        <header className="bg-abyssal-surface/80 glass px-4 py-3 flex items-center gap-3">
  ...
  if (clientError || !client) {
    return (
      <div className="min-h-screen bg-abyssal-bg flex flex-col items-center justify-center gap-4 p-4">
        ...
        <p className="text-[20px] font-bold text-abyssal-text-primary text-center">{clientError || "Cliente no encontrado"}</p>
        <Link href="/clients" className="text-[15px] text-abyssal-primary">Volver a clientes</Link>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-abyssal-bg">
      <header className="bg-abyssal-surface/80 glass sticky top-0 z-40 px-4 py-3 flex items-center justify-between">
        <button onClick={() => router.back()} className="p-2 -ml-2 rounded-full hover:bg-abyssal-surface-high transition-colors active:scale-95">
          <ArrowLeft className="w-5 h-5 text-abyssal-text-secondary" />
        </button>
        <h1 className="text-[17px] font-semibold text-abyssal-text-primary truncate px-2 text-center flex-1">{client.name}</h1>
        <button
          onClick={() => router.push(`/clients/${client.id}/edit`)}
          className="text-[15px] text-abyssal-primary font-semibold hover:opacity-80 transition-opacity active:scale-95"
        >
          Editar
        </button>
      </header>
      ...
      <main className="max-w-[480px] mx-auto p-4 pb-24 space-y-4">
        <section className="relative overflow-hidden bg-abyssal-primary rounded-[2rem] p-6 flex flex-col justify-between min-h-[180px] shadow-abyssal-lg animate-fade-in">
          <div className="relative z-10">
            <span className="text-[12px] text-white/70">Saldo Pendiente</span>
            <p className="text-[28px] text-white font-bold mt-1">{formatCurrency(client.outstanding_balance)}</p>
          </div>
          <div className="relative z-10 flex justify-between items-end">
            <div>
              <span className="text-[12px] text-white/60">Límite de crédito</span>
              <p className="text-[15px] text-white">{formatCurrency(client.credit_limit)}</p>
            </div>
            <button className="bg-white/20 hover:bg-white/30 backdrop-blur-md text-white px-5 py-2 rounded-full text-[15px] font-semibold transition-all active:scale-95">
              Pagar
            </button>
          </div>
        ...
        <section className="grid grid-cols-3 gap-3 animate-fade-in" style={{ animationDelay: "50ms" }}>
          <a href={phoneUrl} className="flex flex-col items-center justify-center bg-abyssal-surface glass rounded-2xl p-4 gap-2 hover:bg-abyssal-surface-high transition-all active:scale-95">
            <div className="w-10 h-10 rounded-full bg-abyssal-green/15 flex items-center justify-center text-abyssal-green">
              <Phone className="w-5 h-5" />
            </div>
            <span className="text-[12px] text-abyssal-text-secondary">Llamar</span>
          </a>
          <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center justify-center bg-abyssal-surface glass rounded-2xl p-4 gap-2 hover:bg-abyssal-surface-high transition-all active:scale-95">
            ...
            <span className="text-[12px] text-abyssal-text-secondary">WhatsApp</span>
          </a>
          <Link href={newOrderUrl} className="flex flex-col items-center justify-center bg-abyssal-primary rounded-2xl p-4 gap-2 hover:opacity-90 transition-all active:scale-95">
            <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center text-white">
              <ShoppingCart className="w-5 h-5" />
            </div>
            <span className="text-[12px] text-white/90">Pedido</span>
          </Link>
        </section>

        <section className="animate-fade-in" style={{ animationDelay: "100ms" }}>
          <h2 className="text-[12px] text-abyssal-text-secondary uppercase tracking-wider mb-3 px-1">Información de Contacto</h2>
          <div className="bg-abyssal-surface glass rounded-2xl overflow-hidden">
            {[
              ...
              <div key={i} className={cn(
                ...
                i < 2 && "border-b border-abyssal-outline/15"
              )}>
                <item.icon className="w-5 h-5 text-abyssal-primary shrink-0" />
                <div>
                  <p className="text-[12px] text-abyssal-text-secondary">{item.label}</p>
                  <p className="text-[15px] text-abyssal-text-primary">{item.value}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="animate-fade-in" style={{ animationDelay: "150ms" }}>
          <div className="flex justify-between items-center mb-3 px-1">
            <h2 className="text-[12px] text-abyssal-text-secondary uppercase tracking-wider">Últimos Pedidos</h2>
            <Link href={`/orders?client_id=${client.id}`} className="text-[15px] text-abyssal-primary">Ver todos</Link>
          </div>
          <div className="space-y-2">
            {ordersLoading ? (
              Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="bg-abyssal-surface glass rounded-2xl p-4 flex items-center justify-between">
              ...
              <div className="bg-abyssal-surface glass rounded-2xl p-6 text-center">
                ...
                <p className="text-[15px] text-abyssal-red">{ordersError}</p>
              </div>
            ) : orders.length === 0 ? (
              <div className="bg-abyssal-surface glass rounded-2xl p-6 text-center">
                <Inbox className="w-10 h-10 text-abyssal-text-secondary mx-auto mb-2" />
                <p className="text-[15px] text-abyssal-text-secondary">Sin pedidos recientes</p>
              </div>
            ) : (
              orders.map((order) => (
                <Link
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl bg-abyssal-surface-high/60 flex items-center justify-center">
                      <Package className="w-6 h-6 text-abyssal-text-secondary" />
                    </div>
                    <div>
                      <p className="text-[15px] text-abyssal-text-primary font-semibold">Orden #{order.order_number}</p>
                      <p className="text-[12px] text-abyssal-text-secondary">{formatDate(order.created_at)}</p>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    <p className="text-[15px] text-abyssal-text-primary font-semibold">{formatCurrency(order.total_value)}</p>
        ...
        <section className="animate-fade-in" style={{ animationDelay: "200ms" }}>
          <h2 className="text-[12px] text-abyssal-text-secondary uppercase tracking-wider mb-3 px-1">Ruta de Entrega</h2>
          <a
            href={mapsUrl}
            target="_blank"
          ...
          >
            ...
            <div className="absolute bottom-4 left-4 flex items-center gap-2 bg-abyssal-surface/80 backdrop-blur-md px-3 py-1.5 rounded-full z-20">
              ...
              <span className="text-[12px] text-abyssal-text-primary">Ver en mapa</span>
            </div>
