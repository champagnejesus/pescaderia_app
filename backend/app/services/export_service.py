...
async def export_clients_csv(db: AsyncSession, business_id: int) -> str:
...
async def export_orders_csv(db: AsyncSession, business_id: int) -> str:
...
async def export_all_zip(db: AsyncSession, business_id: int) -> tuple[bytes, str]:
    buffer = io.BytesIO()
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("productos.csv", await export_products_csv(db, business_id))
        zf.writestr("clientes.csv", await export_clients_csv(db, business_id))
        zf.writestr("pedidos.csv", await export_orders_csv(db, business_id))
    buffer.seek(0)
